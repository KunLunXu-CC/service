...
const parseSchema = async () => {
  ...
  for (const dir of dirs) {
    const resolverPath = new URL(`./schema/${dir}/resolver.mjs`, import.meta.url);
...
export default async (app) => {
  const { graphql: graphqlConfig } = await getConfig();
  ...
  const mapSchemaParams = Object.values(MapperKind).reduce((total, field) => ({
    ...total,
    [field]: (config) => {
      directives[field].forEach((fun) => fun(schema, config));
      return config;
    },
  }), []);
  ...
  // 4. 启动服务
  await server.start();
  server.applyMiddleware({ app, path: graphqlConfig.path });
};
