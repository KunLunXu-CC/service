...
```shell
# 克隆 service 端
git clone https://github.com/qianyin925/blog_service.git
```
...
```shell
cd ../..
git clone https://github.com/qianyin925/blog_client.git
```
...
```shell
rm -rf ../blog_service/docker/nginx/html/*
```

- 拷贝客户端编译好的静态资源到服务端静态资源目录

```shell
cp -R build/* ../blog_service/docker/nginx/html/
```
...
```shell
sudo docker exec blog-service bash -c 'rm -rf /root/*'
```
